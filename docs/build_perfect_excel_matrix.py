...
# Target locations: ONLY docs folder and ZIP
docs_excel = r'C:\Users\jai18\Desktop\TicketForge\docs\Master_168_Hour_Train_Matrix.xlsx'
desktop_root_excel = r'C:\Users\jai18\Desktop\Master_168_Hour_Train_Matrix.xlsx'

# Remove from desktop root if present
if os.path.exists(desktop_root_excel):
    try:
        os.remove(desktop_root_excel)
        print('Removed Excel file from Desktop root as requested.')
    except Exception as e:
        print('Notice removing desktop file:', e)

print('=== BUILDING PERFECT 2-SHEET EXCEL WORKBOOK ===')
...
# Header Row 1: Service Names
# Header Row 2: Day, Hour, Sub-Train Numbers
row1 = ['Day_Name', 'Hour_Of_Day']
row2 = ['Day', 'Hour']

selected_services = list(service_groups.items())[:35] # Top 35 Service Groups
flat_train_list = []

col_cursor = 3

for service_name, t_list in selected_services:
    ...
    # Merge Service Name header if multiple sub-trains
...
print('[1/2] Sheet 1 (Hierarchical 168-Hour Matrix) Built Successfully.')
...
# Add Excel =SUM(C2:C[last]) Formula Row at the Bottom of Column C
...
# Set Generous Column Widths (No Truncation)
...
# Save ONLY to docs folder
wb.save(docs_excel)
print(f'Master Excel File Saved ONLY to Project Docs: {docs_excel}')
